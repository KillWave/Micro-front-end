export default class AopYJS {
  private ctx
  constructor(ctx: any = window) {
    this.ctx = ctx
  }
   ...
   */
  $before(method: string, fn, target = this.ctx) {
    // 闭包保存原方法
    ...
    target[method] = (...args) => {
      ...
      if (Promise && flag instanceof Promise)
        return flag.then((isValid) => isValid && original.apply(target, args))
   ...
   * @param {Object} target 目标对象
   */
  $after(method: string, fn, target = this.ctx) {
    const original = target[method]
